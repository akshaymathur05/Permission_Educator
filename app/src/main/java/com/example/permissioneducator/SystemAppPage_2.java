...
//Implements is for the spinner selection
public class SystemAppPage_2 extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        List<String> systemAppNames = new ArrayList<>();


        for (ApplicationInfo applicationInfo : installedApps) {
            //Instead of using the FLAG_SYSTEM flag, we use the updated one for a more refined sort
            if ((applicationInfo.flags & ApplicationInfo.FLAG_UPDATED_SYSTEM_APP) != 0) {
                ...
                systemAppNames.add(label);
            }
        }

        for (int i = 0; i < systemAppNames.size(); i++) {
            System.out.println(i + ".) System app: " + systemAppNames.get(i) + "\n");
        }

        Spinner spinner = findViewById(R.id.system_app_list);
        ArrayAdapter<String> dataAdapter = new ArrayAdapter<>(this, android.R.layout.simple_spinner_item, systemAppNames);
    ...
    public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
        ...
        //Set up for ListView
        listView = findViewById(R.id.systemPermissionList);
...
//                                Toast.makeText(getApplicationContext(), response.body().string(), Toast.LENGTH_LONG).show();
                                classificationDialog(SystemAppPage_2.this, "Classification of App", response.body().string());
    ...
    system apps, getting those app's permissions, and then putting them in lists to be sent to other
    methods when called
     */
    public List<String> getPermissionList(String nameOfApp, int listType) {
        ...
        List<String> specifiedPermissions = new ArrayList<>();
            ...
            if (requestedPermissions != null && listType == 0) {
                for (int i = 0; i < requestedPermissions.length; i++) {
                    try {
                        ...
                        if ((permissionInfo.protectionLevel == 1 || permissionInfo.protectionLevel == 4097) && packageInfo.packageName.matches(specifiedApp.packageName)) {
                            ...
                            //currentPermission = currentPermission.replace("android.permission.", "");
                            specifiedPermissions.add(currentPermission);
                        }
                    } catch (PackageManager.NameNotFoundException e) {
                        // TODO Auto-generated catch block
                    }
                ...
                for(int i = 0; i < requestedPermissions.length; i++){
                    if(packageInfo.packageName.matches(specifiedApp.packageName)){
                        specifiedPermissions.add(requestedPermissions[i]);
                    }
                }
            }
        }
